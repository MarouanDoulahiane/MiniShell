...
// 	}
// 	i = 0;
// 	while (path[i] && data->command[0][0] != '.' && data->command[0][1] != '/')
// 	{
...
// 	}
// 	free_array(path);
// 	if (!command) // if we have exutable file in the current directory
// 	{
// 		command = ft_strdup(data->command[0]);
// 		if (access(command, X_OK)) // check if we have permission to execute the file if not we return NULL
...
// 	if (!execve_argv || !ft_strcmp(data->command[0], "./")) {
// 		if (data->command[0][0] == '.' && data->command[0][1] == '/' && !access(&data->command[0][2], F_OK))
...
//         return;
//     }
// 	if (!ft_strcmp(data->command[0], "./minishell"))
// 		replace_old_value_if_exist(&data->env, "SHLVL", ft_itoa(ft_atoi(get_the_env(data->env, "SHLVL")) + 1));
// 	tmp = data->env_array;
// 	data->env_array = get_the_env_from_lst(data->env);
// 	free_array(tmp);
// 	if (!ft_strcmp(data->command[0], "./minishell"))
// 		replace_old_value_if_exist(&data->env, "SHLVL", ft_itoa(ft_atoi(get_the_env(data->env, "SHLVL")) - 1));
//     if (pid == 0)
// 	{
//         execve(execve_argv[0], execve_argv, data->env_array);
//         perror("execve");
//         exit(EXIT_FAILURE);
//     }
// 	else
